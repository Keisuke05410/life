...
このディレクトリは、天気・洗濯物の判定ロジック（`weather.ts` とその下位モジュール
`geocode.ts` / `open-meteo.ts` / `weather-code.ts` / `laundry.ts` / `jma.ts`）、
ゴミ出し収集日の算出（`garbage.ts`）と、それらを **1 枚のダッシュボード画像**にして
Discord へ通知する処理（`dashboard.ts` / `render.ts` / `notify.ts`）で構成される。
共通ユーティリティ（JST 日付・時間帯定義・天気ラベル分類・fetch ヘルパー）は `lib.ts` にある。
...
- `src/weather.ts` — `getTodayWeather()`。予報の取得〜サマリ組み立てのオーケストレータ。
- `src/geocode.ts` — 都市名→座標（主要都市エイリアス表 + Open-Meteo ジオコーディング）。
- `src/open-meteo.ts` — Open-Meteo forecast API の型と取得。
- `src/weather-code.ts` — WMO 天気コードの日本語化と家族(family)分類。
- `src/laundry.ts` — 洗濯物の乾きやすさスコア（VPD ベース）と判定。
- `src/jma.ts` — 気象庁の降水確率（6時間ブロック）取得。失敗時は null フォールバック。
- `src/lib.ts` — 共通ユーティリティ（JST 日付 / `PERIOD_DEFS` / `weatherKind` / fetch ヘルパー）。
...
Biome を使う（`biome.json`）。`npm run check` で lint+format 検査、`npm run format` で整形。
push/PR 時に `.github/workflows/ci.yml` が `typecheck` + `biome ci` を実行する。
