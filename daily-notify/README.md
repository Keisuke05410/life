...
その日の **天気・気温（朝/昼/夜）**・**洗濯物アドバイス**・**ゴミ出し（今日／明日）** を、毎朝
...
- ゴミ出しデータ: **武蔵野市の収集ルールをハードコード**（`src/garbage.ts`。外部通信なし・決定的）
- 対象地点: **東京都武蔵野市**（固定）
...
画像（**天気予報がメイン**・洗濯物はサブ・ゴミ出しは最上部バナー）:
- 最上部に **ゴミ出しバナー**（今日＝メイン／明日＝サブ）。収集が無い日は「収集なし」
...
- 例: `☁️ 東京都武蔵野市 曇り 時々 霧雨 24°/21°・洗濯: 乾きにくい(32)・ゴミ: 燃やすごみ`
...
- **ゴミ出し（今日／明日）**:
  武蔵野市（利用者の収集地区）の収集ルールを `src/garbage.ts` にハードコードして算出する。
  毎週の曜日別カテゴリ＋隔週ルール（基準日から14日周期）を合成して当日/翌日の収集を出す。
  収集ルールは**地区・時期によって異なる**ので、引っ越し・制度変更時は `WEEKLY_BY_DOW` と
  `BIWEEKLY`（隔週の基準日）を更新すること（年末年始の休止も同ファイルで例外化）。
...
以上で、毎朝 7:00 JST に自動投稿されます（ワークフロー: `.github/workflows/daily-notify.yml`）。
...
└── src/
    ...
    ├── dashboard.ts  # WeatherResult + GarbageInfo → ダッシュボードHTML（shadcn風ダーク・SVGアイコン）
    ...
    └── garbage.ts    # 武蔵野市の収集ルールから今日/明日のゴミ収集を算出（決定的・外部通信なし）
.github/workflows/daily-notify.yml   # cron（毎朝7:00 JST）+ 手動実行
```
...
- **ゴミ出し**は API ではなく、武蔵野市の収集カレンダー（利用者の収集地区・2026年7月以降）を
  `src/garbage.ts` に**ルールとしてハードコード**している（外部通信なし・決定的）。隔週の収集は
  基準日（例: びん缶＝2026-03-09、燃やさない＝2026-03-11）から14日周期で判定する。
  **地区や制度が変わったら同ファイルを手で更新**する必要がある（自動追従はしない）。
